...
RUN service postgresql start && /bin/su postgres -c "createuser -d -s -r -l ntipa" && /bin/su postgres -c "psql postgres -c \"ALTER USER ntipa WITH ENCRYPTED PASSWORD 'ntipa'\"" && service postgresql stop
